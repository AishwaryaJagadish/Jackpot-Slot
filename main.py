...
def get_bet():
    while True:
        ...
        if amount.isdigit():
            amount = int(amount)
            if MIN_BET_AMT <= amount <= MAX_BET_AMT:
                break
            else:
                print(f"Amount must be between ${MIN_BET_AMT} - ${MAX_BET_AMT} ")
...
def spin(balance):
    ...
    while True:
         ...
         if total_bet > balance:
             print(f"You do not have enought amount to bet. Your current balance is: ${balance}")
         else:
            break
